  ...
  def mlx_import
    ...
    for data in @import_run.input_parsed["data"]
      ...
      for record_id in $record_ids
        ...
        @listings_output[listing]["new"] = l.id.nil?
        @listings_output[listing]["foreign_id"] = foreign_id
        @listings_output[listing]["record_id"] = record_id
        ...
        for key in keys
          ...
          if old_infos[key] != new_infos[key]
            @listings_output[listing]["infos"][key_symbol.to_s] = [old_infos[key], new_infos[key]]
        ...
        if infos_differ or listing.changed?
          ...
          if !listing.errors.empty?
            @listings_output[listing]["errors"] = listings.errors.to_s
          end
        end
        ...
        if (status =="Active-Available" or status == "Active") and !disable?(listing)
          @listings_output[listing]["active"] = true
          active << listing.id
        else
          @listings_output[listing]["active"] = false
        end
        @listings_output[listing]["status"] = status
      end
    end
