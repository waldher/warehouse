...
a.preventDefault();a.stopImmediatePropagation();break;case e.ui.keyCode.PAGE_DOWN:b.nextPage(a);a.preventDefault();a.stopImmediatePropagation();break;case e.ui.keyCode.UP:b.previous(a);a.preventDefault();a.stopImmediatePropagation();break;case e.ui.keyCode.DOWN:b.next(a);a.preventDefault();a.stopImmediatePropagation();break;case e.ui.keyCode.LEFT:b.collapse(a)&&a.stopImmediatePropagation();a.preventDefault();break;case e.ui.keyCode.RIGHT:b.expand(a)&&a.stopImmediatePropagation();a.preventDefault();
break;case e.ui.keyCode.ENTER:if(b.active.children("a[aria-haspopup='true']").length)b.expand(a)&&a.stopImmediatePropagation();else{b.select(a);a.stopImmediatePropagation()}a.preventDefault();break;case e.ui.keyCode.ESCAPE:b.collapse(a)&&a.stopImmediatePropagation();a.preventDefault();break;default:a.stopPropagation();clearTimeout(b.filterTimer);var c;c=b.previousFilter||"";var d=String.fromCharCode(a.keyCode),f=false;if(d==c)f=true;else d=c+d;var g=function(h){return h.replace(/[-[\]{}()*+?.,\\^$|#\s]/g,
...
d=c.prev("a");d.attr("aria-haspopup","true").prepend('<span class="ui-menu-icon ui-icon ui-icon-carat-1-e"></span>');c.attr("aria-labelledby",d.attr("id"))})},focus:function(b,a){var c,d=this;this.blur();if(this._hasScroll()){c=parseFloat(e.curCSS(this.element[0],"borderTopWidth",true))||0;var f=parseFloat(e.curCSS(this.element[0],"paddingTop",true))||0;c=a.offset().top-this.element.offset().top-c-f;f=this.element.scrollTop();var g=this.element.height(),h=a.height();if(c<0)this.element.scrollTop(f+
c);else c+h>g&&this.element.scrollTop(f+c-g+h)}this.active=a.first().children("a").addClass("ui-state-focus").end();d.element.attr("aria-activedescendant",d.active.children("a").attr("id"));this.active.parent().closest(".ui-menu-item").children("a:first").addClass("ui-state-active");d.timer=setTimeout(function(){d._close()},d.delay);c=e(">ul",a);c.length&&/^mouse/.test(b.type)&&d._startOpening(c);this.activeMenu=a.parent();this._trigger("focus",b,{item:a})},blur:function(){if(this.active){clearTimeout(this.timer);
...
"true").position(a)},closeAll:function(){this.element.find("ul").hide().attr("aria-hidden","true").attr("aria-expanded","false").end().find("a.ui-state-active").removeClass("ui-state-active");this.blur();this.activeMenu=this.element},_close:function(){this.active.parent().find("ul").hide().attr("aria-hidden","true").attr("aria-expanded","false").end().find("a.ui-state-active").removeClass("ui-state-active")},collapse:function(b){var a=this.active&&this.active.parents("li:not(.ui-menubar-item)").first();
if(a&&a.length){this.active.parent().attr("aria-hidden","true").attr("aria-expanded","false").hide();this.focus(b,a);return true}},expand:function(b){var a=this,c=this.active&&this.active.children("ul").children("li").first();if(c&&c.length){this._open(c.parent());setTimeout(function(){a.focus(b,c)},20);return true}},next:function(b){this._move("next",".ui-menu-item","first",b)},previous:function(b){this._move("prev",".ui-menu-item","last",b)},first:function(){return this.active&&!this.active.prevAll(".ui-menu-item").length},
...
this.first()?":last":":first"]())},_hasScroll:function(){return this.element.height()<this.element[e.fn.prop?"prop":"attr"]("scrollHeight")},select:function(b){var a={item:this.active};this.closeAll();this._trigger("select",b,a)}})})(jQuery);
