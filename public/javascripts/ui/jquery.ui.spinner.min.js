 ...
 *  jquery.ui.widget.js
 */
(function(e){e.widget("ui.spinner",{version:"1.9pre",defaultElement:"<input>",widgetEventPrefix:"spin",options:{incremental:true,max:null,min:null,numberFormat:null,page:10,step:null,value:null},_create:function(){this._draw();this._markupOptions();this._mousewheel();this._aria()},_markupOptions:function(){var a=this;e.each({min:-Number.MAX_VALUE,max:Number.MAX_VALUE,step:1},function(b,c){if(a.options[b]===null){var d=a.element.attr(b);a.options[b]=typeof d==="string"&&d.length>0?a._parse(d):c}});
this.value(this.options.value!==null?this.options.value:this.element.val()||0)},_draw:function(){var a=this,b=a.options,c=this.uiSpinner=a.element.addClass("ui-spinner-input").attr("autocomplete","off").wrap(a._uiSpinnerHtml()).parent().append(a._buttonHtml()).disableSelection().hover(function(){b.disabled||e(this).addClass("ui-state-hover");a.hovered=true},function(){e(this).removeClass("ui-state-hover");a.hovered=false});this.element.attr("role","spinbutton").bind("keydown.spinner",function(d){if(!b.disabled){if(a._start(d))return a._keydown(d);
...
false)return false;if(!this.counter)this.counter=1;return this.spinning=true},_repeat:function(a,b,c){var d=this;a=a||500;clearTimeout(this.timer);this.timer=setTimeout(function(){d._repeat(40,b,c)},a);d._spin(b*d.options.step,c)},_spin:function(a,b){if(!this.counter)this.counter=1;var c=this.value()+a*(this.options.incremental&&this.counter>20?this.counter>100?this.counter>200?100:10:2:1);c=this._trimValue(c);if(this._trigger("spin",b,{value:c})!==false){this.value(c);this.counter++}},_trimValue:function(a){var b=
this.options;if(a>b.max)return b.max;if(a<b.min)return b.min;return a},_stop:function(a){this.counter=0;this.timer&&clearTimeout(this.timer);this.element.focus();this.spinning=false;this._trigger("stop",a)},_change:function(a){this._trigger("change",a)},_setOption:function(a,b){if(a==="value")b=this._trimValue(this._parse(b));if(a==="disabled")if(b){this.element.attr("disabled",true);this.buttons.button("disable")}else{this.element.removeAttr("disabled");this.buttons.button("enable")}this._super("_setOption",
a,b)},_setOptions:function(a){this._super("_setOptions",a);"value"in a&&this._format(this.options.value);this._aria()},_aria:function(){this.element.attr({"aria-valuemin":this.options.min,"aria-valuemax":this.options.max,"aria-valuenow":this.options.value})},_parse:function(a){if(typeof a==="string")a=e.global&&this.options.numberFormat?e.global.parseFloat(a):+a;return isNaN(a)?null:a},_format:function(a){this.element.val(e.global&&this.options.numberFormat?e.global.format(a,this.options.numberFormat):
a)},destroy:function(){this.element.removeClass("ui-spinner-input").removeAttr("disabled").removeAttr("autocomplete").removeAttr("role").removeAttr("aria-valuemin").removeAttr("aria-valuemax").removeAttr("aria-valuenow");this._super("destroy");this.uiSpinner.replaceWith(this.element)},stepUp:function(a){this._spin((a||1)*this.options.step)},stepDown:function(a){this._spin((a||1)*-this.options.step)},pageUp:function(a){this.stepUp((a||1)*this.options.page)},pageDown:function(a){this.stepDown((a||1)*
this.options.page)},value:function(a){if(!arguments.length)return this._parse(this.element.val());this.option("value",a)},widget:function(){return this.uiSpinner}})})(jQuery);
